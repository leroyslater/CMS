...
export default async function handler(req, res) {
  ...
  const { data: deletedEntries, error: entriesError } = await adminSupabase
    .from("entries")
    .delete()
  ...
  const { data: deletedSessionRows, error: sessionError } = await adminSupabase
    .from("sessions")
    .delete()
    .eq("id", sessionId)
    .select("id");
  ...
  return res.status(200).json({
    deletedSessionIds: (deletedSessionRows || []).map((row) => row.id),
    deletedEntryIds: (deletedEntries || []).map((row) => row.id),
  });
}
